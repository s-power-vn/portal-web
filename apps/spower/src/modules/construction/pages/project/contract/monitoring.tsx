...
import { Loader, SquareMinusIcon, SquarePlusIcon, Table } from 'lucide-react';
import {
  DetailInfoResponse,
  TreeData,
  arrayToTree,
...
const Component = () => {
  ...
  const listDetailInfos = detailApi.listFull.useSuspenseQuery({
    variables: projectId
  });
  ...
  const data = useMemo(
    () =>
      arrayToTree(listDetailInfos.data).sort((v1, v2) =>
        compareVersion(v1.level, v2.level)
      ),
    [listDetailInfos.data]
  );
  ...
  const handleGotoIssue = useCallback(
    (issueId: string) => {
      return navigate({
        to: '/$organizationId/project/$projectId/issues/me/$issueId',
        params: {
          issueId
        }
      });
